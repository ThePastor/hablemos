/* Hablemos service worker — makes the app installable and fully usable offline.
...
const VERSION = 'v31';
const SHELL = 'hablemos-shell-' + VERSION;
const FONTS = 'hablemos-fonts';
...
self.addEventListener('activate', (e) => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys.filter((k) => k.startsWith('hablemos-shell-') && k !== SHELL).map((k) => caches.delete(k)));
...
function offlineCard() {
  return new Response('<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
    ...
    + '<p style="margin:0;opacity:.75">Open Hablemos once with a connection and it will work without one after that.</p></div>',
